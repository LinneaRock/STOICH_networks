...
# Package ID: knb-lter-nwt.10.4 Cataloging System:https://pasta.edirepository.org.
# Data set title: Stream and lake water chemistry data for Green Lakes Valley, 1998 - ongoing..
# Data set creator:  Diane M. Mcknight -  
...
inUrl1  <- "https://pasta.lternet.edu/package/data/eml/knb-lter-nwt/10/4/454485276bd5ee8d4a8a5e30a71853a7" 
infile1 <- tempfile()
...
dt1$conduct <- ifelse((trimws(as.character(dt1$conduct))==trimws("QNS")),NA,dt1$conduct)               
suppressWarnings(dt1$conduct <- ifelse(!is.na(as.numeric("QNS")) & (trimws(as.character(dt1$conduct))==as.character(as.numeric("QNS"))),NA,dt1$conduct))
...
dt1$Cl.hyphen. <- ifelse((trimws(as.character(dt1$Cl.hyphen.))==trimws("QNS")),NA,dt1$Cl.hyphen.)               
suppressWarnings(dt1$Cl.hyphen. <- ifelse(!is.na(as.numeric("QNS")) & (trimws(as.character(dt1$Cl.hyphen.))==as.character(as.numeric("QNS"))),NA,dt1$Cl.hyphen.))
...
dt1$NO3.hyphen. <- ifelse((trimws(as.character(dt1$NO3.hyphen.))==trimws("<0.32")),NA,dt1$NO3.hyphen.)               
suppressWarnings(dt1$NO3.hyphen. <- ifelse(!is.na(as.numeric("<0.32")) & (trimws(as.character(dt1$NO3.hyphen.))==as.character(as.numeric("<0.32"))),NA,dt1$NO3.hyphen.))
...
dt1$PO4.hyphen..hyphen..hyphen. <- ifelse((trimws(as.character(dt1$PO4.hyphen..hyphen..hyphen.))==trimws("QNS")),NA,dt1$PO4.hyphen..hyphen..hyphen.)               
suppressWarnings(dt1$PO4.hyphen..hyphen..hyphen. <- ifelse(!is.na(as.numeric("QNS")) & (trimws(as.character(dt1$PO4.hyphen..hyphen..hyphen.))==as.character(as.numeric("QNS"))),NA,dt1$PO4.hyphen..hyphen..hyphen.))
...
dt1$Si <- ifelse((trimws(as.character(dt1$Si))==trimws("QNS")),NA,dt1$Si)               
suppressWarnings(dt1$Si <- ifelse(!is.na(as.numeric("QNS")) & (trimws(as.character(dt1$Si))==as.character(as.numeric("QNS"))),NA,dt1$Si))
...
dt1$DON <- ifelse((trimws(as.character(dt1$DON))==trimws("QNS")),NA,dt1$DON)               
suppressWarnings(dt1$DON <- ifelse(!is.na(as.numeric("QNS")) & (trimws(as.character(dt1$DON))==as.character(as.numeric("QNS"))),NA,dt1$DON))
...
dt1$TDP <- ifelse((trimws(as.character(dt1$TDP))==trimws("QNS")),NA,dt1$TDP)               
suppressWarnings(dt1$TDP <- ifelse(!is.na(as.numeric("QNS")) & (trimws(as.character(dt1$TDP))==as.character(as.numeric("QNS"))),NA,dt1$TDP))
...
dt1$DOP <- ifelse((trimws(as.character(dt1$DOP))==trimws("QNS")),NA,dt1$DOP)               
suppressWarnings(dt1$DOP <- ifelse(!is.na(as.numeric("QNS")) & (trimws(as.character(dt1$DOP))==as.character(as.numeric("QNS"))),NA,dt1$DOP))
...
dt1$IP <- ifelse((trimws(as.character(dt1$IP))==trimws("QNS")),NA,dt1$IP)               
suppressWarnings(dt1$IP <- ifelse(!is.na(as.numeric("QNS")) & (trimws(as.character(dt1$IP))==as.character(as.numeric("QNS"))),NA,dt1$IP))
...
# keep what we want from these data ####
str(dt1)

lakes <- dt1 |>
  select(local_site, location, depth, year, date, TN, TDN, PN, DON, IN, TP, TDP, PP, DOP, IP) |>
  filter(depth == 0 | is.na(depth))  |>
  filter(!location %in% c('WATERFALL', 'WATERFALL TOP'))

# everything in umol/L except IN, which is in ueq/L

attach(lakes)
summary(as.factor(lakes$local_site))
summary(depth)
summary(year)
summary(date)
summary(TN)
...
summary(IP)
detach(lakes)
write.csv(lakes, 'Data/DataPUllEDI/knb-lter-nwt.10.4_LAKES.csv')
