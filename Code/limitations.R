...
# DIN:TP limitation ####
  ggplot(gl_network) +
  geom_point(aes(log10(TP_umolL), log10(IN_umolL/TP_umolL)), size = 2.5, shape = 21, alpha = 0.5) +
...
# TN:TP limitation ####
ggplot(gl_network) +
  geom_point(aes(log10(TP_umolL), log10(TN_umolL/TP_umolL)), size = 2.5, shape = 21, alpha = 0.5) +
...
# DIN:DIP limitation ####
ggplot(gl_network) +
  geom_point(aes(log10(IP_umolL), log10((IN_umolL/IP_umolL))), size = 2.5, shape = 21, alpha = 0.5) +
  geom_abline(slope = 0, intercept = log10(100), linetype = "dashed") + # Keck and Lepori P limitation line
  geom_abline(slope = 0, intercept = log(1), linetype = "dashed") +  # keck and lepori N limitation line
  geom_abline(slope = 0, intercept = log(16, base = 10), color = "red4") + # redfield
  theme_minimal() +
  labs(y = "Log TN:TP", x = "Log TP") +
  annotate('text', label = 'Redfield 16:1 line', x = -0.5, y = 1.1, hjust = 0, size = 2, color = "red4") +
